...
export const GenerateForgotToken = async (user,email) => {
    const token = await jwt.sign({ userId: user._id },`${FORGET_JWT}${email}`, { expiresIn: '5h' });
    return token;
};

export const VerifyForgotToken = async (token,email) => {
    try {
        if (!token) {
            console.error("No token provided");
            return null;
        ...
        const verified = await jwt.verify(token, `${FORGET_JWT}${email}`);
        return verified;
    } catch (error) {
        console.error("Error verifying token:", error);
        return null; // or handle the error appropriately
